 ...
 * [투표 페이지]
 *
 * 채팅 기반 실시간 투표를 생성·진행·종료·초기화하는 페이지.
 *
 * ── COLORS 배열 ───────────────────────────────────────────────────────────
 *   투표 옵션마다 다른 색상을 할당하기 위한 팔레트.
 *   인덱스 % COLORS.length 로 순환 배정되므로 옵션이 8개를 초과해도 색상이 반복된다.
 *
 * ── handleStart ──────────────────────────────────────────────────────────
 *   /api/voting/start 를 POST 해 서버에서 투표를 시작한다.
 *   빈 옵션 필드가 있으면 전송 전에 필터링하고 최소 2개를 요구한다.
 *
 * ── handleStop ───────────────────────────────────────────────────────────
 *   /api/voting/stop 를 POST 해 투표를 종료한다.
 *   종료 후에도 결과는 voteStore.poll 에 남아 있어 최종 결과를 계속 표시할 수 있다.
 *
 * ── handleClear ──────────────────────────────────────────────────────────
 *   /api/voting/clear 를 POST 해 투표 상태를 idle 로 초기화한다.
 *   서버가 pollUpdate 소켓 이벤트로 idle 상태를 브로드캐스트하면 voteStore 가 갱신된다.
 *
 * ── maxVotes (승자 판별) ─────────────────────────────────────────────────
 *   옵션별 투표 수 중 최대값을 계산해 동점이 아닌 경우 최고득표 항목을 강조한다.
 *   0표일 때도 안전하게 처리하기 위해 Math.max(..., 0) 을 적용한다.
 */
